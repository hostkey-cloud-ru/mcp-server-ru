...
export class InvApiClient {
  ...
  constructor(options: InvApiClientOptions) {
    if (!options.apiKey) {
      throw new InvApiError(
        "РќРµ Р·Р°РґР°РЅ HOSTKEY_API_KEY. РЎРѕР·РґР°Р№С‚Рµ РєР»СЋС‡ РІ InvAPI Рё РїРµСЂРµРґР°Р№С‚Рµ С‡РµСЂРµР· РѕРєСЂСѓР¶РµРЅРёРµ.",
      );
    }
  ...
  private async ensureToken(): Promise<string> {
    ...
    if (typeof token !== "string" || token.length === 0) {
      throw new InvApiError(
        "auth/login РЅРµ РІРµСЂРЅСѓР» СЃРµСЃСЃРёРѕРЅРЅС‹Р№ С‚РѕРєРµРЅ",
        undefined,
        maskSecrets(res),
      );
    }
  ...
  private async rawPost(
    ...
    try {
      ...
      const reason =
        e instanceof Error && e.name === "AbortError"
          ? `С‚Р°Р№РјР°СѓС‚ ${this.httpTimeout}s`
          : String(e);
      throw new InvApiError(`InvAPI ${resource}: СЃРµС‚РµРІР°СЏ РѕС€РёР±РєР° (${reason})`);
    } finally {
      clearTimeout(timer);
    }
    ...
    try {
      ...
      throw new InvApiError(
        `InvAPI ${resource}: HTTP ${res.status}, РЅРµ JSON: ${text.slice(0, 300)}`,
        res.status,
      );
    }
    if (!res.ok) {
