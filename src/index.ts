/** MCP-сервер Hostkey (.ru / invapi.hostkey.ru). Env: см. .env.example. */
...
function intEnv(name: string): number | undefined {
  ...
  if (!Number.isFinite(value) || value <= 0) {
    console.error(
      `hostkey-mcp-server-ru: некорректное ${name}="${raw}", берём значение по умолчанию`,
    );
    return undefined;
  }
  return value;
...
const server = new McpServer(
  { name: "hostkey-mcp-server-ru", version: "0.3.0" },
  {
    instructions:
      "Hostkey InvAPI (.ru). Для записи нужен confirm=true; для опасных — ещё HOSTKEY_ALLOW_DESTRUCTIVE=1. " +
      "Долгие задачи отдают callback — смотри check_task. Есть типизированные инструменты, иначе call_api_raw. " +
      "Промпты: order_server_prompt, reinstall_server_prompt, troubleshoot_server_prompt.",
  },
);
...
console.error("hostkey-mcp-server-ru: готов (stdio)");
